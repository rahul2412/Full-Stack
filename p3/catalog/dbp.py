...
APPLICATION_NAME = "auth (online catalog)"
...
@app.route('/home')
def main_page():
    co=session.query(Country).all()
    c1=session.query(City).all()
    return render_template('main_pages.html' ,countries_name=co,cit_nam=c1)


@app.route('/<int:ids>/')
def main_page1(ids):
    if ids<4:
        c1=session.query(City).filter_by(country_id=ids).all()
...
# deleting cities
@app.route('/<int:ids>/<string:cit>/delete')
def del_city(ids,cit):
    user_id = login_session.get('user_id')
    if user_id is None:
        return redirect('/login')
    c=session.query(City).filter_by(name=cit).one()
...
def add_city(ids):
     return render_template('new_city.html',ids=ids)
...
def add_city1(ids):
    ...
    if names and state and description:
        cityNew = City(
                    name=names,
                    state=state,
...
def edit_city(ids,id2):
    id1=session.query(City).filter_by(id=id2).one()
    user_id = login_session.get('user_id')
    if user_id is None:
        return redirect('/login')
    if id1.user_id != login_session['user_id']:
        return abort(401)
    return render_template('editCity.html',ids=ids,id2=id2,
            cit_nme=id1.name,cit_ste=id1.state,cit_des=id1.description)
    

@app.route('/<int:ids>/<int:id2>/form',methods=['GET', 'POST'])
def edit_city1(ids,id2):
    names=request.form.get('names')
    state=request.form.get('state')
    description=request.form.get('description')
    if names and state and description:
        c3=session.query(City).filter_by(id=id2).one()
        c3.name=names
        c3.state=state
...
def countries_json():
    ...
    return jsonify(Country=[i.serialize for i in countries])
